...
from arguments1 import argparser, logging
...
def reading_our_features(csv_files):
    ...
    for csv_file in sorted(csv_files):  # Sort for consistent ordering
        ...
        if "ProteinID" in df.columns:
            df = df.drop(columns=["ProteinID"])
            print("ProteinID column removed.")
...
def get_aupr(Y, P,threshold):
    ...
    f = open("aupr_davis/P_Y.txt", 'w')
    for i in range(Y.shape[0]):
        f.write("%f %d\n" %(P[i], Y[i]))
    f.close()
    
    f = open("aupr_davis/aupr_metric.txt", 'w')
    subprocess.call(["java", "-jar", "aupr_davis/auc.jar", "aupr_davis/P_Y.txt", "list"], stdout=f)
    f.close()
    
    f = open("aupr_davis/aupr_metric.txt")
    lines = f.readlines()
...
def experiment(FLAGS, perfmeasure, foldcount=6): #5-fold cross validation + test
    ...
    base_path_prot_iFeature = '/content/drive/MyDrive/Kd_Meshari/features/Davis_features_from_domain_seq/'
    ...
    base_path_drug_iFeature = '/content/drive/MyDrive/Kd_Meshari/features/Davis_ligand_features/'
    ...
    csv_files = ['/content/drive/MyDrive/Kd_Meshari/features/Davis_ligand_features/Babel_Chemicals/ligand_descriptors.csv']
    ...
    # ProtBERT
    csv_files = ['/content/drive/MyDrive/Kd_Meshari/features/Davis_features_from_domain_seq/separated_features/ProtBERT/ProtBERT_embeddings.csv']
    ProtBERT = reading_our_features(csv_files)
    print("ProtBERT features = ", ProtBERT.shape)
    # UniRep
    csv_files = ['/content/drive/MyDrive/Kd_Meshari/features/Davis_features_from_domain_seq/separated_features/UniRep/UniRep_jax_embeddings.csv']
    UniRep = reading_our_features(csv_files)
    print("UniRep features = ", UniRep.shape)
    # UniRef50
    csv_files = ['/content/drive/MyDrive/Kd_Meshari/features/Davis_features_from_domain_seq/separated_features/UniRef50/protein_embeddings.csv']
    UniRef50 = reading_our_features(csv_files)
    print("UniRef50 features = ", UniRef50.shape)
    
    # ESM-1b
    csv_files = ['/content/drive/MyDrive/Kd_Meshari/features/Davis_features_from_domain_seq/separated_features/ESM-1b/esm1b_embeddings1.csv']
    ESM_1b = reading_our_features(csv_files)
    print("ESM-1b features = ", ESM_1b.shape)
    # ESM-2
    csv_files = ['/content/drive/MyDrive/Kd_Meshari/features/Davis_features_from_domain_seq/separated_features/ESM-2/esm2_embeddings.csv']
    ESM_2 = reading_our_features(csv_files)
    print("ESM-2 features = ", ESM_2.shape)
    # Morgan_ECFP
    base_path_drug_Morgan_ECFP = '/content/drive/MyDrive/Kd_Meshari/features/Davis_ligand_features/Morgan_ECFP/'
    ...
    # Mordred
    csv_files = ['/content/drive/MyDrive/Kd_Meshari/features/Davis_ligand_features/Mordred/mordred_additional_features.csv']
    ...
    XD = np.concatenate([all_descriptors_drug_iFeature, all_descriptors_drug_Babel, all_descriptors_drug_Morgan_ECFP, all_descriptors_drug_mordred], axis=1)
    ...
    save_path = os.path.join("figures", "Davis_binding_affinity_results.png")
